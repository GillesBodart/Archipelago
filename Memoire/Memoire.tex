...
L'idée de réaliser ce mémoire nous est venue de notre travail en entreprise où nous utilisions un modèle de données très connectées. Nous avions comme mission de remanier ce modèle de données en le rendant totalement historisable. Pour ce faire, nous utilisions une base de données relationnelles mais ce choix nous posait des problèmes, notamment au niveau des dépendances cycliques. Ce genre de difficulté survient quand certains éléments se retrouvent dépendant d'eux-mêmes et forment ainsi un cycle.\textcolor{blue}{article dép cycliques} 

Nous recevions nos informations en provenance de plusieurs sources qui avaient chacune leur propre représentation. Un ensemble de méthodes devait être mise en place et maintenue pour faire correspondre ces modifications avec la représentation interne que l'entreprise avait de l'information. Toutes ces petites modifications rendaient le précédent modèle de moins en moins cohérent et stable. Il nous fallait donc penser à une structure évolutive permettant d'éviter ces ajouts qui alourdissaient les tables pour des demandes spécifiques. 

Suite à ce questionnement, nous nous sommes interrogé d'une part sur la pertinence de notre modèle car nous nous heurtions sans cesse à des difficultés multiples\textcolor{blue}{citer les difficultés} , et d'autre part à \enquote{l'entêtement} de l'utilisation de bases de données relationnelles. 
...
En effet de nos jours, les unités de stockage sont de plus en plus accessibles au grand public, les différentes entreprises impliquées dans le développement des systèmes de gestion de base de données l'ont bien compris, et recherchent des solutions alternatives telles que le NoSql. \textcolor{blue}{source} 

La grande force des SGBDR provient quant à elle, des fondements théoriques mis en place en 1970 par Edgar Frank Codd. IBM définit le langage \enquote{Structured Query Language} (SQL) pour utiliser ce système. Cette normalisation de la représentation de l'information génère un sentiment de sûreté pour les architectes de logiciels.\textcolor{blue}{source} 

A contrario, le mouvement NoSql pour \enquote{Not Only SQL} ne se base pas sur un fondement théorique commun, mais bien sur différentes représentations : Théorie des graphes, systèmes de hash key/value avec partitions, ... \textcolor{blue}{source} 

Ce mémoire va se concentrer sur les implémentations de la théorie des graphes, en essayant de mettre en place un système de comparaison. Nous allons tenter de spécifier certains objectifs communs et développer un framework Java permettant d'utiliser de manière agnostique l'une ou l'autre base de données.\textcolor{blue}{source} 
...
\quad Le framework permettra de faire évoluer la couche de persistance en fonction du modèle de données.
\end{itemize}
